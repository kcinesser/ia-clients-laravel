...
@section('headline')
	<span style="display: block">{{ $remoteDomain->domain }}</span> will expire in {{ $daysOut }} days.
@endsection
...
<main>
    ...
    <div style="background-color: #f3f2ef; border-radius: 4px; padding: 24px 48px;">
        ...
        <!-- Body copy -->
        <p>
            The domain <a href="https://{{ $remoteDomain->domain }}">{{ $remoteDomain->domain }}</a> belonging to {{ $hostedDomain->client->name }} is 
            currently set to expire on <span style="white-space: nowrap;">{{ $remoteDomain->expires->toFormattedDateString() }}</span>. 
        </p>
        ...
        <p>
            When this domain expires, any site or subdomain site tied to this domain will no longer be available on the internet and the client may 
        	lose valuable traffic and incur negative SEO impact. It is also possible for someone to purchase the domain out from under the client once 
        	it is expired.
        </p>
        ...
        <p>
        	...
        	subdomain sites resolve at this domain, please verify that the client no longer needs those sites, and create any offboarding requests 
        	as necessary. 
        </p>
        
        <p>
            If you wish to stop this expiration, <span style="font-style: italic; text-decoration: underline;">action is required on your part.</span> 
